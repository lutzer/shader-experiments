...
float line(const vec2 p1,const vec2 p2, float thickness, const vec2 st) {
    ...
    return smoothstep(thickness + SMOOTHING, thickness, dist);
}
...
void main(){
    ...
    float f = circle(0.403, st) - circle(0.4, st);

    color = mix(color, foreground, f);

    float a = mod(time,TWO_PI);
    f = line(vec2(0.0), vec2(sin(a),cos(a)) * 0.4, STROKE_WIDTH, st);
    color = mix(color, foreground, f);
    // f = line(vec2(0.0,-0.3), vec2(0.0,0.3), STROKE_WIDTH, st);
    // color = mix(color, foreground, f);

    a = mod(time,TWO_PI);
    f = arc(0.45, a, a + 0.5, 0.01, st);
    color = mix(color, foreground, f);
